
package uk.ac.cardiff.nsa.demos.encrypt.aes;

import uk.ac.cardiff.nsa.demos.encrypt.Utils;

public class AESLookupTable {

    final static SBox SBOX = SBox.getInstance();

    public static byte sboxMap(final byte input) {
        final int msb = (input >> 4) & 0x0F;
        final int lsb = input & 0x0F;
        System.out.println("Input byte is " + Utils.byteToBinary(input));
        System.out.println("MSB is " + msb);
        System.out.println("LSB is" + lsb);

        final int converted = SBOX.apply(msb, lsb);
        return (byte) converted;

    }

}

/**
 * Singleton sbox instance
 */
class SBox {

    int[] box = {};

    int[] invBox = {};

    private SBox() {
    }

    public static SBox getInstance() {
        return SBoxHolder.INSTANCE;
    }

    public int apply(final int x, final int y) {
        return box[x * 0x10 + y];
    }

    public int invApply(final int x, final int y) {
        return invBox[x * 0x10 + y];
    }

    private static class SBoxHolder {

        private static final SBox INSTANCE = new SBox();
    }
}